...
class ProgressTrackerService:
    ...
    def _load_real_submissions(self) -> List[Dict[str, Any]]:
        ...
        try:
            submissions_file = os.path.join(os.path.dirname(__file__), '..', 'submissions_data.json')
    ...
    def _get_student_submissions(self, student_identifier: str) -> List[Dict[str, Any]]:
        ...
        for submission in self.submissions_data:
            if (student_identifier.lower() in submission.get('student_name', '').lower() or 
                student_identifier.lower() in submission.get('student_email', '').lower()):
    ...
    def _generate_mock_data(self) -> Dict[str, Any]:
        ...
        submissions = []
        skills = ['Python', 'Java', 'C++', 'Data Structures', 'Algorithms', 'SQL', 'Web Development']
    ...
    def get_skill_analysis(self, student_id: str) -> Dict[str, Any]:
        ...
        sorted_skills = sorted(skill_metrics.items(), key=lambda x: x[1]['average_score'], reverse=True)
    ...
    def get_comparative_analysis(self, student_id: str) -> Dict[str, Any]:
        ...
        current_student_name = student_submissions[0].get('student_name', '') if student_submissions else ''
        ...
        percentile = round((total_students - rank + 1) / total_students * 100, 1) if total_students > 0 else 0
        ...
        return {
            ...
            'performance_comparison': {
                ...
                'percentage_difference': round(((student_avg - class_avg) / class_avg) * 100, 1) if class_avg > 0 else 0
            },
            'peer_comparison': {
                'similar_students': max(1, total_students // 10),
                'better_than': round((total_students - rank) / total_students * 100, 1) if total_students > 0 else 0,
    ...
    def get_achievement_progress(self, student_id: str) -> Dict[str, Any]:
        ...
        if not submissions:
            achievements = [
                {'name': 'First Submission', 'description': 'Complete your first assignment', 'earned': False, 'progress': 0},
                {'name': 'Perfect Score', 'description': 'Get 100% on an assignment', 'earned': False, 'progress': 0},
                {'name': 'Skill Master', 'description': 'Master a programming skill', 'earned': False, 'progress': 0},
                {'name': 'Consistency King', 'description': 'Maintain >80% average', 'earned': False, 'progress': 0}
            ]
        else:
            ...
            achievements = [
                ...
                },
                {
                    ...
                    'date': next((s.get('submitted_at', '')[:10] for s in submissions if s.get('score', 0) >= 100), None) if max_score >= 100 else None,
    ...
    def get_detailed_recommendations(self, student_id: str) -> Dict[str, Any]:
        ...
        if not submissions:
            return {
                ...
                'study_plan': {
                    ...
                    'this_semester': ['Achieve consistent submissions', 'Build programming confidence']
                },
                'resources': [
        ...
        for assignment in assignment_types:
            if 'sort' in assignment and any(s.get('score', 0) < 60 for s in submissions if 'sort' in s.get('assignment_title', '').lower()):
                weak_areas.append('Sorting Algorithms')
            elif 'search' in assignment and any(s.get('score', 0) < 60 for s in submissions if 'search' in s.get('assignment_title', '').lower()):
    ...
    def _calculate_proficiency(self, avg_score: float, avg_attempts: float, submissions: int) -> str:
    ...
    def _group_real_submissions_by_month(self, submissions: List[Dict[str, Any]], months: int) -> List[Dict]:
    ...
    def _group_real_submissions_by_week(self, submissions: List[Dict[str, Any]], weeks: int) -> List[Dict]:
    ...
    def _group_real_submissions_by_day(self, submissions: List[Dict[str, Any]], days: int) -> List[Dict]:
    ...
    def get_all_students_overview(self) -> Dict[str, Any]:
        ...
        for student_name, data in students_data.items():
            average_score = round(data['total_score'] / data['assignment_count'], 1) if data['assignment_count'] > 0 else 0
            ...
            students_list.append({
                ...
                'last_submission': data['last_submission'][:19] if data['last_submission'] else '',
        ...
        return {
            ...
            'class_average': round(sum(s['average_score'] for s in students_list) / len(students_list), 1) if students_list else 0
        }
    ...
    def get_class_statistics(self) -> Dict[str, Any]:
        ...
        if not self.submissions_data:
            return {
                'overview': {'total_students': 0, 'total_submissions': 0, 'class_average': 0, 'pass_rate': 0},
        ...
        unique_students = set(s.get('student_name', '') for s in self.submissions_data if s.get('student_name'))
        all_scores = [s.get('score', 0) for s in self.submissions_data]
        class_average = sum(all_scores) / len(all_scores) if all_scores else 0
        pass_rate = (len([s for s in all_scores if s >= 60]) / len(all_scores)) * 100 if all_scores else 0
        ...
        return {
            ...
            'recent_activity': [
                ...
                } for s in sorted(self.submissions_data, key=lambda x: x.get('submitted_at', ''), reverse=True)[:10]
            ]
        }
    ...
    def delete_student_submissions(self, student_identifier: str) -> bool:
        ...
        try:
            ...
            self.submissions_data = [
                submission for submission in self.submissions_data
                if not (student_identifier.lower() in submission.get('student_name', '').lower() or 
                       student_identifier.lower() in submission.get('student_email', '').lower())
            ]
