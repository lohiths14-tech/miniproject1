    ...
    try:
        ...
        for i, test_case in enumerate(test_cases):
            if not isinstance(test_case, dict) or 'input' not in test_case or 'expected_output' not in test_case:
                return jsonify({'error': f'Test case {i+1} must have input and expected_output'}), 400
...
def get_assignment_submissions(assignment_id):
    try:
        ...
        submissions_data = list(current_app.mongo.db.submissions.find({'assignment_id': assignment_id}))
