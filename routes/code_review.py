...
def get_reviews():
    ...
    try:
        ...
        reviews = review_service.get_reviews(
            status=status, author_id=author_id, reviewer_id=reviewer_id, assignment_id=assignment_id
        )

        return jsonify({"success": True, "data": reviews, "total": len(reviews)})
...
def assign_reviewer(review_id):
    ...
    try:
        ...
        if not reviewer_id:
            return jsonify({"success": False, "error": "Reviewer ID is required"}), 400

        success = review_service.assign_reviewer(review_id, reviewer_id)
        if not success:
            return jsonify({"success": False, "error": "Review not found or already assigned"}), 404

        return jsonify({"success": True, "message": "Reviewer assigned successfully"})
    except (ValueError, KeyError, AttributeError) as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def submit_review(review_id):
    ...
    try:
        ...
        if not success:
            return (
                jsonify({"success": False, "error": "Review not found or not assigned to you"}),
                404,
            )
...
def add_review_comment(review_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": new_comment, "message": "Comment added successfully"}
        )
    except (ValueError, KeyError, AttributeError) as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def approve_review(review_id):
    ...
    try:
        ...
        if not success:
            return (
                jsonify({"success": False, "error": "Review not found or cannot be approved"}),
                404,
            )
...
def reject_review(review_id):
    ...
    try:
        ...
        if not success:
            return (
                jsonify({"success": False, "error": "Review not found or cannot be rejected"}),
                404,
            )

        return jsonify({"success": True, "message": "Review rejected successfully"})
    except (ValueError, KeyError, AttributeError) as e:
        return jsonify({"success": False, "error": str(e)}), 500
