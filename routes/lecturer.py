...
def get_lecturer_dashboard():
    try:
        ...
        assignments_data = list(current_app.mongo.db.assignments.find({'created_by': current_user_id}))
        
        # Get all submissions for these assignments
        assignment_ids = [str(a['_id']) for a in assignments_data]
        submissions_data = list(current_app.mongo.db.submissions.find({
        ...
        dashboard_data['statistics']['average_score'] = total_score / graded_count if graded_count > 0 else 0
        dashboard_data['statistics']['plagiarism_violations'] = plagiarism_violations
        
        # Sort recent submissions by date
        dashboard_data['recent_submissions'].sort(key=lambda x: x.get('submitted_at', datetime.min), reverse=True)
        dashboard_data['recent_submissions'] = dashboard_data['recent_submissions'][:10]  # Last 10 submissions
...
def get_students():
    try:
        ...
        students_data = list(current_app.mongo.db.users.find(
            {'role': 'student'},
        ...
        for student in students_data:
            ...
            submissions_data = list(current_app.mongo.db.submissions.find({'student_id': student_id}))
...
def get_analytics():
    try:
        ...
        # Get assignments created by this lecturer
        assignment_ids = [str(a['_id']) for a in current_app.mongo.db.assignments.find({'created_by': current_user_id})]
        ...
        for submission in submissions_data:
            ...
            if submitted_at:
                ...
                analytics['submission_trends'][date_key] = analytics['submission_trends'].get(date_key, 0) + 1
        ...
        if scores:
            ...
            analytics['performance_metrics']['standard_deviation'] = statistics.stdev(scores) if len(scores) > 1 else 0
...
def get_leaderboard():
    try:
        ...
        for student in students_data:
            ...
            if submissions_data:
                average_score = sum(s.get('score', 0) for s in submissions_data) / len(submissions_data)
